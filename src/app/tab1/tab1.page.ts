...
export class Tab1Page {
  ...
  {
    this.loading = true;
    this.db.collection<Article>('articles',ref=>{
  ...
  {    
    ...
    if(status)
    {
      this.db.collection('articles').doc(data.id).update({
  ...
  async editArticle(data) {
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        ...
        icon: 'trash',
        handler: () => {
          var conf = confirm('Hapus artikel?');
          if(!conf) return;
          //delete image
          if(data.image != undefined)
          this.storage.ref(data.image.ref).delete();
          //delete data
          this.db.collection('articles').doc(data.id).delete().then(res=>{
  ...
  buatKegiatan()
  {
    this.router.navigate(['form-kegiatan/0']);
  }
